...
import com.laptopstore.app.databinding.FragmentAdminProductListBinding;
...
public class AdminProductListFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        binding.fabAddProduct.setOnClickListener(v -> {
            Navigation.findNavController(view).navigate(R.id.action_adminProductListFragment_to_adminProductDetailFragment);
        });
    }
    ...
    private void setupRecyclerView() {
        adapter = new AdminProductAdapter(product -> {
            ...
            Navigation.findNavController(requireView()).navigate(R.id.action_adminProductListFragment_to_adminProductDetailFragment, bundle);
        });
    ...
    private void loadProducts() {
        ...
        ApiClient.getApiService(requireContext()).searchProducts("", 0, 50).enqueue(new Callback<ApiResponse<PagedResponse<Product>>>() {
            @Override
            public void onResponse(Call<ApiResponse<PagedResponse<Product>>> call, Response<ApiResponse<PagedResponse<Product>>> response) {
                ...
                if (response.isSuccessful() && response.body() != null && response.body().isSuccess()) {
                    adapter.setProducts(response.body().getData().getContent());
                } else {
